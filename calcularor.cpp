...
//'!n' is transfered into functiion factor： tgamma(n+1)
...
queue <string> postFixQueue(queue <string>& stringToOrganize){
    ...
    while(!stringToOrganize.empty()){
        ...
        if (front!="+" && front!="-" && front!="*" && front!="/" && front!= "^" && front!= "%" && front!= "!"){
            postfix.push(front);
        }
        
        else if (front=="+" || front=="-" || front=="*" || front=="/" || front== "^" || front== "%" || front=="!"){
            if (op.empty()){
                op.push(front);
            }
            else if (!op.empty()){
        ...
        if (curr!="+" && curr!="-" && curr!="*" && curr!="/" && curr!= "^" && curr!="%" && curr!="!"){
            ...
            switch(operation){
                ...
                case '%':
                    ...
                    second = oprand.top();
                    oprand.pop();
                    tempResult = (double)fmod(second, first);
                    oprand.push(tempResult);
                    break;
                ...
                case '!':
                    ...
                    second = oprand.top();
                    oprand.pop();
                    tempResult = (double)factorial(second);
                    oprand.push(tempResult);
                    break;
...
string stringCheck(std::string input) {
    ...
    // Iterate through the string and replace if necessary
    for (size_t i = 1; i < input.length(); ++i) {
        if (input[i - 1] == '(' && (input[i] == '+' || input[i] == '-')) {
            input.replace(i, 1, "0" + std::string(1, input[i]));
        }
        if(input[i - 1] == '!' ) {
            input.replace(i, 1, "1"+string(1, input[i]));
        }
    }
    // 判断输入字符串是否以 ! 结尾
    if (input.back() == '!') {
...
int main(){
    string quit = "quit";
    mMap["PI"] = 3.14159265358979323846;
    mMap["E"] = 2.71828182845904523536;
    
    //store user input into a string called input
    string input;
    getline(cin, input);
    input = stringCheck(input);
    int i, j;
        ...
        if (firstFourChar == "let "){
            ...
            for(int i=0;i<assignExpression.length();i++){
                ...
                if (assignExpression[i]!='+' && assignExpression[i]!='-' && assignExpression[i]!='*' && assignExpression[i]!='/' && assignExpression[i]!=' ' && assignExpression[i]!='^' && assignExpression[i]!='!' && assignExpression[i]!='%' && assignExpression[i] !='(' && assignExpression[i]!=')') {
                    substring+=assignExpression[i];
                }
                else if(assignExpression[i]!='+' || assignExpression[i]!='-' || assignExpression[i]!='*' || assignExpression[i]!='/' || assignExpression[i]!='^' || assignExpression[i]!='%' || assignExpression[i]!='!' || assignExpression[i] !='(' || assignExpression[i]!=')'){
                    if (substring!="") {
                ...
                if (hasVariable && !foundVarable) {
                    ...
                    if (invalid) {
                        ...
                        if(fabs(result) >= 1e6) {
                            ...
                            cout << setprecision(6) << result << endl; // Set precision to 6 and use fixed format
                        }
                    }
                }
            }
